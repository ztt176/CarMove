 ...
 * @LastEditTime: 2021-10-14 17:20:05
...
<div class="app-container">
    ...
    <el-table :data="recordList" v-loading="loading" style="width: 100%">
        ...
        <el-table-column prop="name" label="记录名称" align="center" show-overflow-tooltip>
        </el-table-column>
        ...
        <el-table-column prop="distance" label="覆盖范围(平方公里)" align="center">
        </el-table-column>
    ...
    <el-dialog title="新增走航记录" :visible.sync="airborenOpen" width="600px" append-to-body>
        <el-form :model="airborneForm" ref="airborneForm" :rules="rules" label-width="80px">
            ...
            <el-form-item label="服务目标" label-width="80px" prop="serviceId">
                <el-select v-model="airborneForm.serviceId" placeholder="请先选择所属客户" size="mini">
            ...
            <el-form-item label="计划任务" label-width="80px" prop="taskId">
                <el-select v-model="airborneForm.taskId" placeholder="请先选择服务目标" size="mini">
                    <el-option v-for="(item,index) in taskOptions" :value="item.id" :key="index" :label="item.name"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="无人机" label-width="80px" prop="moveId">
                <el-select v-model="move" placeholder="请选择" size="mini" value-key="id" @change="moveChange">
            ...
            <el-form-item label="记录名称" label-width="80px" prop="name">
                <el-input v-model="airborneForm.name"></el-input>
            </el-form-item>
            <el-form-item label="关联数据" label-width="80px" v-if="airborneForm.deptId&&airborneForm.moveId" prop="savePath">
                <el-input v-model="airborneForm.savePath" disabled></el-input>
                <el-radio-group v-model="airborneForm.savePath">
        ...
        <div slot="footer">
            <el-button @click="airborenOpen = false;airborneForm = {};dept={}">取 消</el-button>
...
export default {
    ...
    computed: {
        serviceid() {
            return this.airborneForm.serviceId
        }
    },
    watch: {
        airborneForm: { //监听表单属性
    ...
    methods: {
        ...
        addSailing() {
            ...
            this.airborenOpen = true;
        },
        deptIdChange(val) {
            this.airborneForm.deptId = val.deptId;
            listDeptService(val.deptId).then((res) => {
                this.serviceList = res.rows;
            })
        },
        moveChange(val) {
            this.airborneForm.moveId = val.id;
            if (this.dept.deptName && this.move.code) {
                let param = {
                    deptName: this.dept.deptName,
                    bizMoveCode: this.move.code,
                    mode: '飞行'
        ...
        //提交新增的走航记录
        submitSailing() {
            this.$refs['airborneForm'].validate((valid) => {
                if (valid) {
                    addRecord(this.airborneForm).then(res => {
                        this.airborenOpen = false;
                        this.getList()
                        this.$message({
                            type: 'sucess',
                            message: '新增成功'
